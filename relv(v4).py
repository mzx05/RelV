...
class Node:
    ...
    metadata: Dict = None
    age: int = 0
    creation_step = 0   # passed in during node creation
    last_reference_step = 0  # initialize to creation

    def __init__(self, content: str, embedding: np.ndarray, node_id: str = None, metadata: Dict = None, age: int = 0,current_query_step=0):
        self.id = node_id if node_id else str(uuid.uuid4())
        self.content = content
        self.embedding = embedding
        self.metadata = metadata if metadata else {}
        self.age = age
        self.creation_step = current_query_step  # passed in during node creation
        self.last_reference_step = current_query_step  # initialize to creation
...
class RelV:
    ...
    def __init__(self, embedding_dim: int = 768, similarity_threshold: float = 0.5, chunk_size: int = 512, overlap: int = 50,
        ...
        self.query_step = 0  # initialize logical time
    ...
    def advance_time(self):
        self.query_step += 1
    def test_age(self):
        age = self.compute_effective_age(
            t_now=1000,
            t_created=900,
            num_references=2,
            t_last_ref=950,
    ...
    def add_node(self, content: str, embedding: np.ndarray, metadata: Dict = None) -> str:
        ...
        """
        cur_step = self.query_step
        # Create a new node
        node = Node(content=content, embedding=embedding, metadata=metadata,current_query_step = cur_step)
        node_id = node.id
    ...
    def compute_effective_age_steps(self,query_step,created_at_step,last_reference_step,num_references,lambda_ref=0.1,mu_freshness=2.0,beta_decay=0.05):
        """
        Computes the effective age of a node, factoring in reference count and freshness decay.

        Parameters:
            t_now (float): Current time (e.g., timestamp).
            t_created (float): Node's creation time.
            num_references (int): Number of times the node has been referenced.
            t_last_ref (float): Timestamp of the node's last reference.
            lambda_ref (float): Weight controlling reference count effect.
            mu_freshness (float): Weight controlling freshness bonus.
            beta_decay (float): Rate of exponential decay for freshness.

        Returns:
            float: The effective age score (lower = fresher/more relevant).
        """
        query_step = self.query_step
        age_steps = query_step - created_at_step
        delta = query_step - last_reference_step
        denominator = (1 + lambda_ref * num_references) * (1 + mu_freshness * math.exp(-beta_decay * delta))
        return age_steps / denominator
    def update_node_ages(self, max_age: int = 5, delete_old_nodes: bool = False) -> List[str]:
        current_step = self.query_step
        deleted_nodes = []

        reference_counts = {}
        last_reference_steps = {}

        # First pass: count references
        for node_id in self.nodes:
            reference_counts[node_id] = 0
            last_reference_steps[node_id] = self.nodes[node_id].last_reference_step

        for node_id in self.nodes:
            node = self.nodes[node_id]
            for related_id, _ in self.get_related_nodes(node_id):
                reference_counts[related_id] += 1
                last_reference_steps[related_id] = max(
                    last_reference_steps[related_id],
                    node.creation_step  # Or current_step if that's more appropriate
                )

        for node_id in list(self.nodes.keys()):
            node = self.nodes[node_id]
            effective_age = self.compute_effective_age_steps(
                query_step=current_step,
                created_at_step=node.creation_step,
                last_reference_step=last_reference_steps[node_id],
                num_references=reference_counts[node_id],
                lambda_ref=0.1,
                mu_freshness=2.0,
                beta_decay=0.05
            )

            node.age = effective_age

            if delete_old_nodes and effective_age > max_age:
...
class RelVContextManager:
    ...
    def add_conversation_to_memory(self, query: str, response: str) -> str:
              ...
              # Commented out because we maintain the database in the query_optimized function every time
              """if len(self.query_history) % 2 == 0:  # Every 2 queries
        ...
        for batch_idx in range(0, total_chunks, batch_size):
            ...
            for i, (chunk, embedding) in enumerate(zip(current_batch, batch_embeddings)):
                ...
                node_id = self.relv_db.add_node(
                    ...
                    current_query_step=self.relv_db.query_step
    ...
    def maintain_database(self, max_age: int = 5, auto_cleanup: bool = True) -> Dict:
        """
        ...
        print("\nSample of node ages after maintenance:")
        for i, (node_id, node) in enumerate(self.relv_db.nodes.items()):
            print(f"  Node {node_id}: age = {node.age:.3f}, created at = {node.creation_step}, last ref = {node.last_reference_step}")
            if i >= 4:
                break  # limit to first 5
...
"""
    ...
    def query_optimized(self, query: str, k: int = 3, cache_results: bool = True,
        ...
        if use_batched_context:
            ...
            for node_id, similarity in relevant_nodes:
                #EDITED HERE(IN CASE CREATES ERROR): RESET RELEVENT NODE AGES
                self.relv_db.reset_node_age(node_id)
                node_content = self.relv_db.get_node_content(node_id)
                node = self.relv_db.get_node(node_id)
                node.last_reference_step = self.relv_db.query_step
                source = get_node_source(node)
                source_info = f" | Source: {source}"
...
def create_qa_dataset(context_manager, document_text, document_id="document",
                     char_interval=5000, questions_per_interval=1,
                     output_file="qa_dataset.json", difficulty="medium"):
    """
    Create a question-answer dataset from a raw document text.

    Args:
        context_manager: The RelVContextManager instance with access to the LLM
        document_text: Raw text of the document to generate questions from
        document_id: Identifier for the document (for metadata purposes)
        char_interval: Number of characters between question generation points
        questions_per_interval: Number of questions to generate per text interval
    ...
    # Create a prompt for question generation based on difficulty
    difficulty_descriptions = {
        "easy": "straightforward factual questions that can be directly answered from the text",
        "medium": "questions that require understanding the content and making simple connections",
        "hard": "challenging questions that require deep understanding, inference, or connecting multiple concepts"
    }
    ...
    if not document_text or not isinstance(document_text, str):
        print("Error: Invalid document text provided")
        return {"document": document_id, "total_questions": 0, "total_intervals": 0}

    # Use the raw document text directly
    full_text = document_text
    print(f"Document contains {len(full_text)} characters")

    # Create a dataset for QA pairs
    qa_dataset = []

    # Determine interval positions
    num_intervals = max(1, len(full_text) // char_interval)
    print(f"Creating questions at {num_intervals} intervals (~every {char_interval} characters)")

    # Process each text interval
    for i in range(num_intervals):
        start_pos = i * char_interval
        end_pos = min(start_pos + char_interval * 2, len(full_text))  # Get a bit more context

        # Extract context for this interval
        context = full_text[start_pos:end_pos]

        print(f"Processing interval {i+1}/{num_intervals} (chars {start_pos}-{end_pos})")

        # Ensure the context isn't too long for the LLM
        max_context_len = min(4000, context_manager.max_context_size // 2)
        if len(context) > max_context_len:
            # Truncate and add an indicator
            context = context[:max_context_len] + "... [truncated]"

        # Generate questions for this interval
        print(f"  Generating {questions_per_interval} questions...")

        # Create a prompt for generating questions
        prompt = f"""Generate {questions_per_interval} {difficulty} questions based on the following text context.

For each question, provide:
1. The question
2. The correct answer based solely on the provided context
3. The specific context snippet that contains the answer

Make sure each question:
- Is answerable from the given context
- Is a {difficulty_desc}
- Has a clear, unambiguous answer

FORMAT: Output as a JSON array of objects with the fields: "question", "answer", "context_snippet"
...
QUESTIONS:"""
        ...
        try:
            response = context_manager.llm(prompt, max_tokens=2048, temperature=0.7)
            ...
            try:
                ...
                if isinstance(questions, list):
                    for q_data in questions:
                        if isinstance(q_data, dict) and "question" in q_data and "answer" in q_data:
                            # Add metadata
                            q_data["document"] = document_id
                            q_data["char_position"] = start_pos
                            q_data["interval_index"] = i
                            q_data["full_context"] = context

                            # Leave an empty field for actual answer during evaluation
                            q_data["actual_answer"] = ""

                            # Add to dataset
                            qa_dataset.append(q_data)
            except json.JSONDecodeError:
                print(f"  Error: Could not parse JSON from LLM response")
                print(f"  Raw response: {generated_content[:4000]}...")

                # Attempt to salvage questions from the text if JSON parsing failed
                ...
                for line in lines:
                    line = line.strip()
                    if line.startswith(('Q:', 'Question:', '1.', '2.', '3.')) and 'Answer:' not in line:
                        ...
                        if current_q and current_a:
                            qa_dataset.append({
                                ...
                                "interval_index": i,
                                "context_snippet": "",  # We don't have this from non-JSON format
                        ...
                        # Start new question
                        current_q = line.split(':', 1)[1].strip() if ':' in line else line.lstrip('123456789. ')
                        current_a = None
                    elif line.startswith(('A:', 'Answer:')) and current_q:
                        current_a = line.split(':', 1)[1].strip()

                    if current_q and current_a and question_count < questions_per_interval:
                          qa_pair = {
                              "question": current_q,
                              "answer": current_a,
                              "document": document_id,
                          ...
                          # Real-time display of final salvaged question-answer pair
                          print(f"\n===== Salvaged Q&A Pair #{len(qa_dataset)} =====")
                          print(f"Question: {qa_pair['question']}")
                          print(f"Answer: {qa_pair['answer']}")
                          print(f"Position: Character {start_pos}, Interval {i}")
                          print("=====================================")
        except Exception as e:
            print(f"  Error generating questions: {e}")

        # Brief delay to avoid LLM rate limits
        time.sleep(1)
    ...
    try:
        ...
        print(f"\nSuccessfully saved {len(qa_dataset)} QA pairs to {output_file}")
    except Exception as e:
...
def evaluate_qa_model(qa_dataset_path, relv_db, context_manager, output_file=None, use_llm_grading=True):
    """
    Evaluate a model on the QA dataset by having it answer the questions
    and comparing to expected answers. Can use LLM to grade responses.

    Args:
        qa_dataset_path: Path to the QA dataset JSON file
        relv_db: The RelV database instance
        context_manager: The RelVContextManager instance with access to the LLM
        output_file: Path to save the results JSON (if None, modifies the input file)
        use_llm_grading: Whether to use the LLM to grade responses (vs simple string matching)
    ...
    try:
        ...
        print(f"Loaded {len(qa_dataset)} questions from {qa_dataset_path}")
    except Exception as e:
    ...
    for i, qa_item in enumerate(qa_dataset):
        ...
        context = qa_item.get("context_snippet", "")

        # If no specific context snippet, use the full context
        if not context and "full_context" in qa_item:
            context = qa_item["full_context"]

        print(f"\nProcessing question {i+1}/{len(qa_dataset)}: {question}")

        # Create a prompt that includes both the question and context
        prompt = f"""Answer the following question based on the provided context. Be concise and direct.
...
Answer:"""
        ...
        # Get model's answer
        try:
            response = context_manager.query_optimized(prompt,temperature=0.1)
            actual_answer = response.strip()

            # Store the actual answer in the dataset
            qa_item["actual_answer"] = actual_answer

            # Grade the answer
            if use_llm_grading:
                # Use LLM to grade the response
                grade_prompt = f"""You are an expert grader evaluating question answering systems. Your task is to grade the answer to a question based on the expected answer.
...
Explanation: [brief explanation]"""
                ...
                try:
                    grade_response = context_manager.llm(grade_prompt, max_tokens=256, temperature=0.1)
                    grade_text = grade_response.strip()

                    # Extract score using regex
                    score_match = re.search(r'Score:\s*(0\.\d+|[01](\.\d+)?)', grade_text)
                    if score_match:
                        score = float(score_match.group(1))

                        # Extract explanation
                        explanation_match = re.search(r'Explanation:\s*(.*)', grade_text, re.DOTALL)
                        explanation = explanation_match.group(1).strip() if explanation_match else "No explanation provided"

                        # Add to metrics
                        qa_item["score"] = score
                        qa_item["grading_explanation"] = explanation
                        total_score += score

                        # Categorize the score
                        if score >= 0.8:
                            correct_count += 1
                            qa_item["evaluation"] = "correct"
                        elif score >= 0.5:
                            partially_correct_count += 1
                            qa_item["evaluation"] = "partially_correct"
                        else:
                            qa_item["evaluation"] = "incorrect"
                    else:
                        # Fallback if regex fails
                        print("  Could not extract score from LLM grading response")
                        qa_item["score"] = 0.5
                        qa_item["grading_explanation"] = "Error in score extraction"
                        qa_item["evaluation"] = "error"
                except Exception as e:
                    print(f"  Error during LLM grading: {e}")
                    # Fall back to simple string matching
                    use_llm_grading = False

            # Use simple string comparison if LLM grading fails or is disabled
            if not use_llm_grading:
                if actual_answer.lower() == expected_answer.lower():
                    correct_count += 1
                    qa_item["evaluation"] = "correct"
                    qa_item["score"] = 1.0
                elif expected_answer.lower() in actual_answer.lower() or actual_answer.lower() in expected_answer.lower():
                    partially_correct_count += 1
                    qa_item["evaluation"] = "partially_correct"
                    qa_item["score"] = 0.5
                else:
                ...
                total_score += qa_item["score"]

            print(f"  Expected: {expected_answer}")
            print(f"  Actual: {actual_answer}")
            print(f"  Score: {qa_item['score']:.2f}")
            if "grading_explanation" in qa_item:
                print(f"  Explanation: {qa_item['grading_explanation']}")
        except Exception as e:
            print(f"  Error getting answer: {e}")
            qa_item["actual_answer"] = f"ERROR: {str(e)}"
            qa_item["evaluation"] = "error"
            qa_item["score"] = 0.0
    ...
    partial_accuracy = (correct_count + partially_correct_count) / total if total > 0 else 0
    ...
    output_path = output_file if output_file else qa_dataset_path
    try:
    ...
    print(f"  Partially correct: {metrics['partially_correct']} ({metrics['partially_correct']/total:.2%})")
    print(f"  Combined accuracy: {metrics['partial_accuracy']:.2%}")
    print(f"  Average score: {metrics['average_score']:.2f}")

    return metrics

# Example usage:
# 1. Create a QA dataset from your RelV database
# qa_stats = create_qa_dataset(relv_db, context_manager, pages_per_source=3, questions_per_page=2,
#                             output_file="qa_dataset.json", difficulty="medium")

# 2. Evaluate the model on the dataset
# eval_metrics = evaluate_qa_model("qa_dataset.json", relv_db, context_manager)

qa_stats = create_qa_dataset(context_manager, merger_text, char_interval=40000, questions_per_interval=1,
                     output_file="qa_dataset.json", difficulty="medium")
#Char interval is 40000 because approx len of doc is 800000 chars and we want approx 20 questions in the dataset

eval_metrics = evaluate_qa_model("qa_dataset.json", relv_db, context_manager)

"""**EXPERIMENT 2: CROSS-DOCUMENT RETRIEVAL/LOGIC**"""

def create_cross_document_qa_dataset(context_manager, document_text, document_id="document",
                     char_interval=5000, questions_per_interval=1,
                     output_file="qa_dataset.json", difficulty="medium",
                     synthetic_context_file="synthetic_context.txt"):
    """
    Create a question-answer dataset from a raw document text with additional info and synthetic context.
    ...
    Args:
        ...
        output_file: Path to save the JSON output file
        difficulty: Difficulty level of questions (easy, medium, hard)
        synthetic_context_file: Path to save the synthetic context
    ...
    # Create a file to store all synthetic context
    all_synthetic_context = ""
    ...
    for i in range(num_intervals):
        ...
        # Create a prompt for generating questions with additional info and synthetic context
        prompt = f"""Generate {questions_per_interval} {difficulty} questions based on the following text context.
...
2. The correct answer based solely on the provided context
3. The specific context snippet that contains the answer
4. Additional Info: Create additional information or background that would make this question more challenging
5. Synthetic Context: Write 1-2 paragraphs of synthetic text that incorporates both the original context and the additional info to provide a more comprehensive background

Make sure each question:
- Is answerable from the given context
- Is a {difficulty_desc}
- Has a clear, unambiguous answer
- Has additional info that extends beyond what's directly in the context
- Has synthetic context that sensibly incorporates both the original context snippet and the additional info

FORMAT: Output as a JSON array of objects with the fields: "question", "answer", "context_snippet", "additional_info", "synthetic_context"
...
QUESTIONS:"""
        ...
        try:
            ...
            try:
                ...
                if isinstance(questions, list):
                    for q_data in questions:
                        if isinstance(q_data, dict) and "question" in q_data and "answer" in q_data:
                            ...
                            # Ensure additional_info and synthetic_context exist
                            if "additional_info" not in q_data:
                                q_data["additional_info"] = "No additional information provided."

                            if "synthetic_context" not in q_data:
                                q_data["synthetic_context"] = "No synthetic context generated."

                            # Add each synthetic context to the collection with a header
                            question_header = f"\n\n--- SYNTHETIC CONTEXT FOR QUESTION {len(qa_dataset) + 1} ---\n"
                            all_synthetic_context += question_header + q_data["synthetic_context"] + "\n"
                ...
                current_synthetic_context = []
                reading_synthetic = False
                question_count = 0

                for line in lines:
                    line = line.strip()

                    # Detect question, answer, or other components
                    if line.startswith(('Q:', 'Question:', '1.', '2.', '3.')) and 'Answer:' not in line:
                        # Save previous Q&A if we have them
                        if current_q and current_a:
                            synthetic_text = "\n".join(current_synthetic_context) if current_synthetic_context else "No synthetic context generated."

                            qa_pair = {
                                "question": current_q,
                                "answer": current_a,
                                "context_snippet": current_snippet or "",
                                "additional_info": current_additional_info or "No additional information provided.",
                                "synthetic_context": synthetic_text,
                                "document": document_id,
                                "char_position": start_pos,
                                "interval_index": i,
                                "full_context": context,
                            ...
                            # Add to all synthetic context
                            question_header = f"\n\n--- SYNTHETIC CONTEXT FOR QUESTION {len(qa_dataset) + 1} ---\n"
                            all_synthetic_context += question_header + synthetic_text + "\n"
                    ...
                    elif line.startswith(('Context Snippet:', 'Context:', '3.')) and current_q and current_a:
                        current_snippet = line.split(':', 1)[1].strip() if ':' in line else line.lstrip('123456789. ')

                    elif line.startswith(('Additional Info:', 'Additional Information:', '4.')) and current_q and current_a:
                        current_additional_info = line.split(':', 1)[1].strip() if ':' in line else line.lstrip('123456789. ')

                    elif line.startswith(('Synthetic Context:', 'Synthetic:', '5.')) and current_q and current_a:
                        reading_synthetic = True
                        remaining_text = line.split(':', 1)[1].strip() if ':' in line else line.lstrip('123456789. ')
                        if remaining_text:
                            current_synthetic_context.append(remaining_text)

                    elif reading_synthetic and line and current_q and current_a:
                        # Continue reading synthetic context until we hit another section
                        if not line.startswith(('Q:', 'Question:', '1.', '2.', '3.', '4.', '5.')):
                            current_synthetic_context.append(line)
                ...
                if current_q and current_a and question_count < questions_per_interval:
                    ...
                    print(f"Question: {qa_pair['question']}")
                    print(f"Answer: {qa_pair['answer']}")
                    print(f"Additional Info: {qa_pair['additional_info']}")
                    print(f"Position: Character {start_pos}, Interval {i}")
                    print("=====================================")
        except Exception as e:
            print(f"  Error generating questions: {e}")
    ...
    # Save all synthetic context to a separate file
    try:
        with open(synthetic_context_file, 'w', encoding='utf-8') as f:
            f.write(f"# Synthetic Context for {document_id}\n")
            f.write(f"# Generated on {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(all_synthetic_context)
        print(f"\nSuccessfully saved synthetic context to {synthetic_context_file}")
    except Exception as e:
        print(f"Error saving synthetic context to file: {e}")
...
cross_qa_stats = create_cross_document_qa_dataset(context_manager, document_text= merger_text, char_interval=40000, questions_per_interval=1, output_file="cross_qa_dataset.json", difficulty="medium")
ans = evaluate_cross_document_qa(qa_dataset_path="cross_qa_dataset.json", relv_db=relv_db, context_manager=context_manager, use_llm_grading=True)

"""PROCESS SYNTHETIC TEXT and MERGER TEXT"""

with open("/content/synthetic_context.txt", "r") as f:
  synth_text = f.read()
process_and_add_document_with_textrank(synth_text,context_manager,metadata={"source": "synthetic_valmet_context"})
process_and_add_document_with_textrank(merger_text,context_manager,metadata={"source":"merger_text"})

del context_manager.query_cache

ans = evaluate_cross_document_qa(qa_dataset_path="cross_qa_dataset.json", relv_db=relv_db, context_manager=context_manager, use_llm_grading=True)

eval_metrics = evaluate_qa_model("cross_qa_dataset.json", relv_db, context_manager)
